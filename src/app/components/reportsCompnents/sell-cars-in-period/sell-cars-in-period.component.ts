...
@Component({
  selector: 'app-sell-cars-in-period',
  templateUrl: './sell-cars-in-period.component.html',
  styleUrl: './sell-cars-in-period.component.css'
})
export class SellCarsInPeriodComponent  implements OnInit{
...
if(adddateForm.value.startAddedday=="اليوم"||adddateForm.value.startAddedmonth=="الشهر"||
   ...
   this.getSellCarsInPeriod(this.startDate,this.endDate);
   this.error=""
...
getSellCarsInPeriod(starDateFrom:string,endDateFrom:string){
  this._ReportsService.getSellCarsInPeriod(starDateFrom,endDateFrom).subscribe((res)=>{
